...
def list_usecases():
    ...
    try:
        usecases = usecase_service.get_all_usecases_with_details(session)
...
def view_usecase(usecase_id):
    session = SessionLocal()
    try:
        usecase = usecase_service.get_usecase_by_id(session, usecase_id)
        if not usecase:
        ...
        other_usecases = usecase_service.get_all_other_usecases(session, usecase_id)
...
def edit_usecase(usecase_id):
    ...
    try:
        ...
        if request.method == 'POST':
            success, message = usecase_service.update_usecase_from_form(session, usecase, request.form)
...
def delete_usecase(usecase_id):
    ...
    try:
        ...
        uc_name, message = usecase_service.delete_usecase_by_id(session, usecase_id)
...
def inline_update_usecase(usecase_id):
    session = SessionLocal()
    try:
        usecase = session.query(UseCase).get(usecase_id)
        ...
        updated_usecase, message, updated_data = usecase_service.inline_update_usecase_field(session, usecase, field, value)
        if updated_usecase:
