...
(function() { // <-- FIX: Add opening IIFE
    ...
    window.usecaseExplorer.initializeUsecaseOverview = function() {
        // ... (the entire body of the function remains exactly the same) ...
        // --- START REFACTOR: Read from Data Island ---
        const dataIsland = document.getElementById('page-data-island');
        const pageData = dataIsland ? JSON.parse(dataIsland.textContent) : {};

        const allUsecasesDataForJS = pageData.usecases || [];
        const allStepsDataForJS = pageData.steps || [];
        // --- END REFACTOR ---
        ...
        function populateUsecaseFilterDropdown(container) {
            ...
            if (filterType === 'stepName') {
                ...
                uniqueValues = stepsForFilter.map(step => ({
                        id: step.id.toString(),
                        name: `${step.name} (BI_ID: ${step.bi_id})` // Assuming steps data from API has bi_id
                    }))
